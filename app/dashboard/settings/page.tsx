  ...
  return <div className="page-shell py-10">
    ...
    <div className="mx-auto mt-6 max-w-3xl">
      ...
      <form action={updateAccount} className="premium-panel mt-5 space-y-5 rounded-lg p-6 sm:p-8">
        <label className="block text-sm font-bold"><span className="flex items-center gap-2"><UserRound size={16} /> Display name</span><input name="display_name" defaultValue={displayName} required minLength={2} className="mt-2 min-h-12 w-full rounded-md border border-white/10 bg-black/20 px-4 outline-none focus:border-[#8b5cf6]" /></label>
        <label className="block text-sm font-bold"><span className="flex items-center gap-2"><Mail size={16} /> Email</span><input defaultValue={userEmail} readOnly className="mt-2 min-h-12 w-full cursor-not-allowed rounded-md border border-white/[.06] bg-black/10 px-4 text-[#8991a6]" /></label>
        <label className="block text-sm font-bold">
          <span className="flex items-center justify-between">
            <span>WhatsApp number</span>
            {savedWhatsapp ? (
              <span className="text-xs text-[#00d68f] font-bold font-mono">✓ Linked (+{savedWhatsapp})</span>
            ) : (
              <span className="text-xs text-amber-400 font-bold font-normal">Not linked yet</span>
            )}
          </span>
          <input
            name="whatsapp"
            defaultValue={savedWhatsapp}
            inputMode="tel"
