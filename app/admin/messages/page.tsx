...
export default async function AdminMessagesPage({ searchParams }: { searchParams: Promise<{ prefill?: string }> }) {
  ...
  const [{ data: customers }, { data: games }] = await Promise.all([supabase.from("profiles").select("id,display_name,whatsapp,email").eq("role", "customer").order("display_name"), supabase.from("games").select("id,title").eq("archived", false).order("title")]);
