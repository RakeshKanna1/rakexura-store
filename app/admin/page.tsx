    ...
    <div className="py-4 md:py-8">
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-2 md:mt-3 text-3xl font-black md:text-5xl bg-gradient-to-r from-white via-[#e8e3ff] to-[#b9a4ff] bg-clip-text text-transparent">
            Rakexura control center
          </h1>
          <p className="section-copy text-[#9ea6b9] text-xs md:text-sm mt-1">
            Run catalog, orders, promotions, and customer trust without touching code.
          </p>
        </div>
        <span className="flex min-h-9 md:min-h-11 items-center self-start rounded-md border border-[#00d68f]/15 bg-[#00d68f]/[.05] px-3 md:px-4 text-[11px] md:text-xs font-bold text-[#70efbb]">
      ...
      <section className="mt-5 md:mt-6 grid grid-cols-2 gap-2.5 md:grid-cols-3 lg:grid-cols-6">
        {quick.map(([href, label, Icon]) => (
          <Link
            key={href}
            href={href}
            className="flex min-h-20 md:min-h-24 flex-col justify-between rounded-md border border-[#8b5cf6]/20 bg-[#0d0b1a]/85 p-3 md:p-4 text-xs md:text-sm font-black transition duration-300 hover:-translate-y-0.5 hover:border-[#b9a4ff]/30 hover:shadow-[0_8px_20px_rgba(139,92,246,0.04)]"
          >
            <Icon size={18} className="text-[#b9a4ff] filter drop-shadow-[0_0_4px_rgba(139,92,246,0.3)]" />
            {label}
          </Link>
        ))}
      </section>
      
      <div className="mt-6 md:mt-8 grid grid-cols-2 gap-2.5 md:gap-3 lg:grid-cols-4">
        {stats.map(([label, value, Icon], idx) => {
          ...
          return (
            <article
              className={`premium-panel rounded-md p-4 md:p-5 bg-[#0f0c22]/80 border-[#8b5cf6]/20 transition duration-300 hover:-translate-y-1 ${hoverClass}`}
              key={label}
            >
              <Icon className={`mb-3 md:mb-6 ${iconClass}`} />
              <strong className="block text-xl md:text-2xl text-white">{value}</strong>
              <span className="muted mt-1 text-[11px] md:text-xs">{label}</span>
            </article>
          );
        })}
      </div>
      
      <div className="mt-6 md:mt-8 grid gap-4 lg:grid-cols-[1fr_320px]">
        {/* Added min-w-0 to prevent layout overflow from the inner table min-w-[650px] on mobile devices */}
        <section className="min-w-0">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="section-title text-lg md:text-xl">Latest orders</h2>
            <Link href="/admin/orders" className="text-xs md:text-sm font-bold text-[#b9a4ff] hover:text-[#c4b5fd] transition-colors">
              Open queue →
            </Link>
          </div>
          ...
          <div className="space-y-2.5 md:hidden">
            {latestOrders?.map((order) => (
              <Link
                key={order.id}
                href="/admin/orders"
                className="block rounded-xl border border-white/10 bg-[#0d0b1a]/90 p-3.5 space-y-2 active:bg-white/[0.04] transition"
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono font-bold text-white text-xs">
                    {order.order_reference || `#${order.id}`}
                  </span>
                  <span className="font-black text-[#facc15] text-xs">
                    {formatPrice(order.total_price)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-2 text-xs">
                  <span className="text-[#8991a6] truncate">{order.customer_name || "Customer"}</span>
                  <span className="rounded px-2 py-0.5 text-[10px] font-bold bg-white/10 text-white/90 border border-white/15">
                    {order.order_status}
                  </span>
                </div>
              </Link>
