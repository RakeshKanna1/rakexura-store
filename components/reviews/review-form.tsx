...
export function ReviewForm({ gameId, gameTitle }: { gameId: number; gameTitle: string }) {
  ...
  return <section className="space-y-3"><OnboardingHint id="first-review" title="Reviews and feedback">Share your thoughts about this game. Submissions are moderated before publication.</OnboardingHint><div className="premium-panel rounded-xl p-4 sm:p-6"><div className="flex items-start gap-3"><BadgeCheck className="mt-1 shrink-0 text-[#00d68f]" /><div><p className="eyebrow">Game Review</p><h2 className="mt-2 text-xl font-black">Review {gameTitle}</h2><p className="mt-2 text-sm leading-6 text-[#8991a6]">Reviews and optional media are moderated before publication.</p></div></div><form onSubmit={submitReview} className="mt-6 space-y-4"><fieldset><legend className="mb-2 text-xs font-bold text-[#aeb5c8]">Your rating</legend><div className="flex gap-2">{Array.from({ length: 5 }, (_, index) => index + 1).map((value) => <button key={value} type="button" suppressHydrationWarning onClick={() => setRating(value)} aria-label={`${value} stars`} className="rounded p-1 transition hover:scale-110 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#facc15]"><Star size={22} className={value <= rating ? "text-[#ffb800]" : "text-[#4b5265]"} fill={value <= rating ? "currentColor" : "none"} /></button>)}</div></fieldset><label htmlFor="review-message" className="sr-only">Review Message</label><textarea id="review-message" name="message" value={message} onChange={(event) => setMessage(event.target.value)} minLength={10} maxLength={1200} required rows={4} placeholder="Share your delivery and game experience" className="w-full resize-y rounded-md border border-white/10 bg-black/25 p-4 text-sm outline-none transition focus:border-[#facc15]/60" /><label className="flex min-h-14 cursor-pointer items-center gap-3 rounded-md border border-dashed border-white/10 bg-black/15 px-4 text-sm transition hover:border-white/25"><Paperclip size={17} className="text-[#facc15]" /><span><b className="block">{media.length ? `${media.length} file${media.length > 1 ? "s" : ""} selected` : "Add optional proof media"}</b><small className="text-[#8991a6]">Up to 2 images or MP4 files, 10 MB each</small></span><input type="file" multiple accept="image/jpeg,image/png,image/webp,video/mp4" className="sr-only" onChange={(event) => selectMedia(event.target.files)} /></label><div className="flex flex-wrap items-center justify-between gap-3"><Link href={loginHref} className="text-xs text-[#8991a6] hover:text-white">Need to sign in?</Link><button type="submit" disabled={submitting || message.trim().length < 10} className="btn btn-primary gap-2 disabled:cursor-not-allowed disabled:opacity-50"><Send size={15} /> {submitting ? "Submitting..." : "Submit review"}</button></div></form></div></section>;
}
