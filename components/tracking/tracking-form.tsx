...
type TrackedOrder = { order_id: number; order_ref: string; status: string; total_price: number; created_at: string; items: Array<{ title: string; platform: string }>; customer_name: string };
    ...
    <div>
      <div className="glass rounded-lg p-5">
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          <label className="text-xs font-bold text-[#aeb5c8]">
            Order reference
            <input value={order} onChange={(event) => setOrder(event.target.value)} placeholder="RKX-2606-000123" className="mt-2 h-12 w-full rounded-md border border-white/10 bg-black/25 px-4 text-sm outline-none focus:border-[#facc15]" />
          </label>
          <label className="text-xs font-bold text-[#aeb5c8]">
            WhatsApp number
            <input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="91 98765 43210" inputMode="tel" className="mt-2 h-12 w-full rounded-md border border-white/10 bg-black/25 px-4 text-sm outline-none focus:border-[#facc15]" />
          </label>
