  ...
  return (
    <section className="my-4 sm:my-8 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/[.08] bg-white/[.04] lg:grid-cols-4 shadow-[inset_0_1px_1px_rgba(255,255,255,0.06)]">
      {stats.map(({ icon: Icon, value, suffix, label }) => (
        <article key={label} className="relative overflow-hidden border border-amber-500/5 backdrop-blur-md p-3 sm:p-5 md:p-6 transition-all duration-300 hover:shadow-[inset_0_0_12px_rgba(250,204,21,0.04)] group" style={{ background: "linear-gradient(145deg, rgba(7, 7, 8, 0.95) 0%, rgba(22, 18, 10, 0.05) 100%)" }}>
          ...
          <Icon size={18} className="mb-2 sm:mb-5 text-[#facc15] filter drop-shadow-[0_0_4px_rgba(250,204,21,0.2)]" />
          <strong className="block text-lg sm:text-2xl md:text-3xl font-black text-white leading-tight">
            <AnimatedCounter value={value} suffix={suffix} />
          </strong>
          <span className="mt-0.5 sm:mt-1 block text-[11px] sm:text-xs text-[#8991a6] font-medium leading-snug">{label}</span>
        </article>
      ))}
    </section>
  );
