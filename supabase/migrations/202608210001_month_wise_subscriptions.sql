-- Migration: 202608210001_month_wise_subscriptions.sql
-- Description: Add month-wise subscription pricing (price_1m, price_2m, price_3m, price_6m, price_12m)
-- and update create_store_order RPC to verify duration-based platform pricing.

-- 1. Add subscription duration pricing columns to games
alter table public.games
add column if not exists price_1m numeric default null,
add column if not exists price_2m numeric default null,
add column if not exists price_3m numeric default null,
add column if not exists price_6m numeric default null,
add column if not exists price_12m numeric default null;

-- 2. Update create_store_order function with month-wise pricing validation
...
begin
  -- Idempotency check for payment proof screenshot path
  if p_payment_proof_path is not null and nullif(trim(p_payment_proof_path), '') is not null then
    select order_reference into v_reference from public.orders where screenshot_url = nullif(trim(p_payment_proof_path), '') limit 1;
  ...
  if p_payment_reference is not null and nullif(trim(p_payment_reference), '') is not null then
    select order_reference into v_reference from public.orders where payment_reference = nullif(trim(p_payment_reference), '') limit 1;
    if v_reference is not null then
      return v_reference;
    end if;
  end if;
  ...
  select coalesce(sum(case item->>'platform'
    when '1 Month' then coalesce(g.price_1m, g.xbox_price, g.steam_price, g.sale_price, 0)
    when '2 Months' then coalesce(g.price_2m, 0)
    when '3 Months' then coalesce(g.price_3m, 0)
    when '6 Months' then coalesce(g.price_6m, 0)
    when '12 Months' then coalesce(g.price_12m, 0)
  ...
  where coalesce(g.archived, false) = false
    and (
      item->>'platform' = any(coalesce(g.available_platforms, array['Steam','Epic','Offline','Online','Xbox','Nvidia GeForce','1 Month','2 Months','3 Months','6 Months','12 Months']))
      or (coalesce(g.is_subscription, false) = true and item->>'platform' in ('1 Month', '2 Months', '3 Months', '6 Months', '12 Months', 'Xbox', 'Nvidia GeForce'))
    );
  ...
  if nullif(trim(p_coupon_code), '') is not null then
      ...
      begin
        ...
        for v_line in select jsonb_array_elements(p_items) loop
          ...
          select case v_line_platform 
            when '1 Month' then coalesce(price_1m, xbox_price, steam_price, sale_price, 0)
            when '2 Months' then coalesce(price_2m, 0)
            when '3 Months' then coalesce(price_3m, 0)
            when '6 Months' then coalesce(price_6m, 0)
            when '12 Months' then coalesce(price_12m, 0)
    ...
    -- Block free game triggers for ranks below Diamond (points < 4000) UNLESS a valid active coupon was applied
    if (v_total - v_discount) = 0 then
      ...
      if v_coupon_id is null then
        ...
        v_current_points := coalesce(v_current_points, 0);
        if v_current_points < 4000 then
          raise exception 'Free game checkouts are restricted to Diamond and Platinum loyalty ranks or valid promo codes.';
        end if;
      end if;
    end if;
  end if;
