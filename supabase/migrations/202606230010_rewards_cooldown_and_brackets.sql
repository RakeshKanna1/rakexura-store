-- Phase 19: Add last_request_date cooldown field and align points brackets schema
alter table public.profiles add column if not exists last_request_date timestamptz;

-- Re-create refresh_reward_level() with consistent point brackets matching dashboard
...
begin
  new.level := case
    ...
    when new.points >= 4000 then 'Diamond'
...
-- Re-create create_store_order to check point boundaries consistently at 4000 points
...
begin
  ...
  if v_coupon_id is not null then
    -- Decrement global stock (usage limit) integer by 1, and flag as inactive globally if it reaches zero or less
    update public.coupons
    set usage_limit = case when usage_limit is not null then usage_limit - 1 else usage_limit end,
        active = case when usage_limit is not null and usage_limit - 1 <= 0 then false else active end
...
-- Run sync points trigger recalculation
