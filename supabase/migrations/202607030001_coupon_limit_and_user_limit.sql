-- Migration 202607030001_coupon_limit_and_user_limit.sql
-- Relaxes coupon_usage constraint to allow guest users to register usage (user_id drop NOT NULL).
-- Implements global usage limit counting from coupon_usage table.
-- Implements flexible per-user limit checking against coupons.per_user_limit.
-- Automatically notifies the owner when a coupon is used.
...
begin
  ...
  if nullif(trim(p_coupon_code), '') is not null then
    ...
    -- Check global usage limit by counting entries in coupon_usage
    select count(*) into v_coupon_uses from public.coupon_usage where coupon_id = v_coupon_id;
    if v_coupon_usage_limit is not null and v_coupon_uses >= v_coupon_usage_limit then
      raise exception 'This coupon code has reached its global usage limit and is expired.';
    end if;

    -- Enforce flexible per-user limit
    if auth.uid() is not null then
      if (select count(*) from public.coupon_usage where coupon_id = v_coupon_id and user_id = auth.uid()) >= coalesce(v_coupon_per_user, 1) then
        raise exception 'You have already redeemed this coupon the maximum allowed times (% time(s)).', coalesce(v_coupon_per_user, 1);
      end if;
    end if;
  ...
  if v_coupon_id is not null then
    -- Record usage (works for guests too since user_id is now nullable)
    insert into public.coupon_usage (coupon_id, user_id, order_id) values (v_coupon_id, auth.uid(), v_id);

    -- Deactivate coupon if global usage limit is reached after this checkout
    if v_coupon_usage_limit is not null then
      select count(*) into v_coupon_uses from public.coupon_usage where coupon_id = v_coupon_id;
      if v_coupon_uses >= v_coupon_usage_limit then
        update public.coupons set active = false where id = v_coupon_id;
      end if;
    end if;

    -- Notify the owner of the coupon usage
    ...
    if v_owner is not null then
      ...
      values (
        v_owner,
        'Coupon code used',
        coalesce(p_customer_name, 'A customer') || ' used coupon code ' || upper(trim(p_coupon_code)) || ' on order ' || v_reference || '.',
        'promotion',
        '/admin/orders'
      );
    end if;
    
    -- Handle point reset burn cycle (only for logged-in users)
    if auth.uid() is not null then
      if upper(trim(p_coupon_code)) in ('DIAMONDFREE', 'DIAMOND-FREEBIE') then
        select points into v_current_points from public.user_rewards where user_id = auth.uid();
