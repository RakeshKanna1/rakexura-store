-- Migration 202606250001_checkout_and_requests.sql
-- Enforces RAKETHREE minimum selection constraint and PLATINUM-FREEBIE quantity-based pricing rules.

-- Also update support_tickets status constraint to allow 'Approved' if needed
alter table public.support_tickets drop constraint if exists support_tickets_status_check;
alter table public.support_tickets add constraint support_tickets_status_check check (status in ('open', 'in_progress', 'resolved', 'closed', 'Approved'));
      ...
      begin
        ...
        for v_line in select jsonb_array_elements(p_items) loop
          ...
          select case v_line_platform 
            when 'Epic' then epic_price 
            when 'Offline' then offline_price 
            else steam_price end into v_line_price
